// Rasterises the mascot into the favicons: assets/favicon.ico, favicon-96.png
// and apple-touch-icon.png, all copied into web/public by assets/build.sh sync.
//
...
// The mascot's own ground (the <rect> in icon.svg, and --bg-primary in the CSS).
const INK = "#0a0c10";

const svg = readFileSync(join(REPO, "assets", "icon.svg"), "utf8");

/** The mascot at `px` square, as PNG bytes. */
function render(px) {
...
// its own corner mask, and the mascot's rounded <rect> leaves the four corners
// of the canvas transparent, which iOS composites against black — a dark ring
// around a dark icon. Filling them with the mascot's own ground makes the seam
